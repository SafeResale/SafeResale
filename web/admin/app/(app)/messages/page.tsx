    ...
    <div>
        ...
        <>
          <div className="overflow-hidden rounded-lg border">
            <Table>
              ...
              <TableBody>
                {data.items.map((m) => (
                  <TableRow key={m._id} className="cursor-pointer" onClick={() => open(m)}>
                    <TableCell>
                    ...
                    <TableCell><StatusBadge tone={statusTone[m.status] || "neutral"} label={m.status} /></TableCell>
      ...
      <Dialog open={!!selected} onOpenChange={(v) => { if (!v) { setSelected(null); setDetail(null); } }}>
        <DialogContent className="sm:max-w-xl">
          ...
          <div className="max-h-72 overflow-y-auto whitespace-pre-wrap rounded-lg border bg-muted/40 p-4 text-sm">
            {detail?.message || "No body."}
          </div>
          <DialogFooter className="gap-2 sm:gap-2">
            <Button variant="outline" size="sm" onClick={() => remove(selected!)}><Archive className="size-3.5" /> Delete</Button>
            {detail?.status === "resolved" ? (
              <Button size="sm" onClick={() => setState(selected!, "archived")}><CheckCheck className="size-3.5" /> Archive</Button>
            ) : (
              <Button size="sm" onClick={() => setState(selected!, "resolved")}><CheckCheck className="size-3.5" /> Mark resolved</Button>
            )}
          </DialogFooter>
        </DialogContent>
      </Dialog>
