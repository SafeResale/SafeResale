...
import { fmtShort, riskLabel, timeAgo } from "@/lib/format";
        ...
        <>
          <div className="divide-y overflow-hidden rounded-lg border">
            {data.items.map((it) => {
              ...
              return (
                <Link key={it.decision._id} href={`/listings/${it.listing?._id || it.decision.listing_id}`} className="flex items-center gap-4 px-4 py-3 transition-colors hover:bg-accent/50">
                  <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                    <p className="truncate text-sm font-medium">{it.listing?.title || "Untitled listing"}</p>
                    <p className="text-xs text-muted-foreground">
                      {it.listing?.category || "—"} · created {timeAgo(it.listing?.created_at)} · flagged {timeAgo(it.decision.created_at)}
                    </p>
                  </div>
                  {risk.band && <StatusBadge tone={riskTone[risk.band]} label={it.risk?.badge ? `${it.risk.badge} (${risk.label})` : `${risk.label} risk`} />}
                  <StatusBadge tone={statusTone[it.decision.status] || "warning"} label={it.decision.status} />
